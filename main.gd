...
func _on_timer_timeout() -> void:
	if !$Garden/UpgradeSpot/UpgradeIndicator.visible:
		...
		$HUD/ProgressBar.value = (1 - time_to_upgrade/(float(upgrade_base_timer + (upgrades * 5))))*100
...
func upgrade_update(upgrade_amount : int):
	...
	time_to_upgrade = upgrade_base_timer + (upgrades * 5)
...
func toggle_pause():
	...
	if get_tree().paused:
		$HUD/PauseScrene/Button.grab_focus()
